...
const Res = () => {
	...
	const editHandler = () => {
		...
		dispatch(todoActions.editTodo({task : editTask ,id : task.id}))
	}
		...
		<EditBlock>
			<Flex direction = {'column'} align = {'center'} justify = {'space-around'}>
				<div>
					{!isValid && (
						<Alert severity='error' className='alertEdit'>
							please enter somethig..
						</Alert>
					)}
					<Backdrop />
					<h1 className='editzogo'>UPDATE TASK</h1>
					<Input
						isValid={isValid}
						className='input'
						type='text'
				...
				<Flex direction={'row'} justify={'center'} align = {'center'}>
					<ButtonUi onClick={editHandler}>edit</ButtonUi>
					<ButtonUi onClick = {()=>dispatch(todoActions.hideEdit())}>confirm</ButtonUi>
				</Flex>
			</Flex>
